...
function initArrayBuffer(gl, attribName, attribUseSize, type, size) {
    const buffer = gl.createBuffer();
...
function initAttributeVariable(gl, a, d) {
    if (!a)
        return;
...
function render() {
    ...
    setMatrix(gl);
    gl.drawElements(gl.TRIANGLES, vertexData.index.count, vertexData.index.type, 0);
}
function updateFPS() {
...
function main() {
    ...
    of.loadAndParse().then(({ positions, normals, colors, indices }) => {
        console.log("positions, normals, colors, indices", positions, normals, colors, indices);
...
window.addEventListener("load", (e) => {
    main();
    function modifyEyeAngle(x, y) {
        function canChange(v) {
            const { val } = v;
    ...
    canvas.addEventListener("mousemove", (e) => {
        if (isMouseDown) {
            if (mouseBtn === 2)
                modifyEyeAngle(e.clientX, e.clientY);
            if (mouseBtn === 0)
                modifyModelAngle(e.clientX, e.clientY);
        }
    });
});
