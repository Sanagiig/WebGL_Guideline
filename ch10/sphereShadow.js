...
function generateSphereVertex() {
    ...
    const ysr = getRad(ystart), yer = getRad(yend), xsr = getRad(xstart), xer = getRad(xend);
    const yang = yer - ysr;
    const xang = xer - xsr;
    const positionList = [];
    const indexList = [];
...
function initPlaneVertexBuffers(gl, isShadow = false) {
    // Vertex coordinates
...
function initTriangleVertexBuffers(gl, isShadow = false) {
    // var vertices = new Float32Array([
    ...
    const a_Position = initArrayBuffer(gl, "a_Position", vertices, 3, gl.FLOAT, 0, 0);
...
function initSphereVertexBuffers(gl, isShadow = false) {
...
function setMatrix(gl, isShadow = false) {
    const u_MvpMatrix = gl.getUniformLocation(gl.program, "u_MvpMatrix");
    const u_NormalMatrix = gl.getUniformLocation(gl.program, "u_NormalMatrix");
    const u_ShadowMvpMatrix = gl.getUniformLocation(gl.program, "u_ShadowMvpMatrix");
    const m = new Matrix().setRotate(globalSetting.ang, 0, 1, 0);
    const vp = isShadow
        ? new Matrix()
            .setPerspective(45, 1, 1.0, 1000.0)
            .lookAt(lightPosition[0], lightPosition[1], lightPosition[2], 0, 0, 0, 0, 1, 0)
        : new Matrix()
            .setPerspective(45, 1, 1.0, 1000.0)
            .lookAt(eyePosition[0], eyePosition[1], eyePosition[2], 0, 0, 0, 0, 1, 0);
    m.multiply(vp);
    if (isMouseDown)
        console.log("mvp Matrix-------------------", m.originData());
    if (!isShadow) {
        ...
        if (isMouseDown)
            console.log("------------- shadow light matrix", shadowM.originData());
...
function main() {
    ...
    shadowProgram = createProgram(gl, getShadowVShaderSource(), getShadowFShaderSource());
...
function updateLimit(id) {
    ...
    if (after.length) {
        const originEle = document.getElementById(id);
        const effectEle = document.getElementById(id.replace(after[0], after[1]));
...
function updateTag(notUpdateSpan = false) {
    const inputIds = [
        "accumulate",
        "ystart",
        "yend",
    ...
    inputIds.forEach((id) => {
        const inputEle = document.querySelector(`#${id}`);
        if (!notUpdateSpan) {
...
window.addEventListener("load", (e) => {
    ...
    (_a = document.getElementById("colorCheck")) === null || _a === void 0 ? void 0 : _a.addEventListener("change", (e) => {
        const { gl } = globalSetting;
        const check = e.target.checked ? 1 : 0;
        let u_HasColor;
        gl.program = shadowProgram;
        u_HasColor = gl.getUniformLocation(gl.program, "u_HasColor");
