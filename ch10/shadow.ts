...
function main() {
  ...
  function initTriangleVetexBuffers(
    gl: WebGLRenderingContext,
    program: WebGLProgram,
  ...
  function drawTriangle(
    ...
    gl.uniformMatrix4fv(
      uniformLocation,
      false,
      globalModelMatrix.multiply(vpMatrix).originData(),
    );
  ...
  const shadowProgram = createProgram(
    gl,
    getVShaderShadowSource(),
    getFShaderShadowSource(),
  );
  const normalProgram = createProgram(
    gl,
    getVShaderSource(),
    getFShaderSource(),
  );
  ...
  const shadow_u_MvpMatrix = gl.getUniformLocation(
    shadowProgram,
    "u_MvpMatrix",
  )!;
  const u_MvpMatrix = gl.getUniformLocation(normalProgram, "u_MvpMatrix")!;
  const u_MvpMatrixFromLight = gl.getUniformLocation(
    normalProgram,
    "u_MvpMatrixFromLight",
  )!;
